...
10 Thoughts (4 full articles + 6 PDF publications), the corporate profile and
10-year anniversary report as Documents, and the featured homepage Announcement.
PDFs are vendored in `seed-assets/`. The script is idempotent — existing records
(matched by slug/title) are skipped, so it is safe to run on every deploy.
